...
public:
    void log(uint8_t instr, uint16_t param_addr, struct CpuContext const * const context);
};

// ---------------------------------------------------------------------------------------------- //
void StdOutLogger::log(uint8_t instr, uint16_t param_addr, struct CpuContext const * const context) {
    std::cout << boost::format("%-06X%-03X%-03X%-03X")
    % context->PC % static_cast<int>(instr) % (param_addr & 0xFFU) % ((param_addr & 0xFF00) >> 8) ;
